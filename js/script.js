...
var points = textToPoints("2025 蛇年大吉！", 20, "Anton");
    ...
    if (this.vy >= 0) {
      ...
      if (this.title) {
        if(ts < 1) {
          ...
          var color = [255, 215, 0];//[Math.random() * 256 >> 0, Math.random() * 256 >> 0, Math.random() * 256 >> 0];
          for (var i = 0; i < Math.PI * 2; i += 0.05) {
            var power = (Math.random() + 0.4) * 10;
...
function Particle(x, y, vx, vy, radius, color, time = 1) {
  this.x = x;
  this.y = y;
  this.vx = vx;
  this.vy = vy;
